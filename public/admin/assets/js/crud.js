...
function status_update(url,newStatus, id) {
  ...
  if (statusText === 8 || statusText === 3 ){
    var statusText =  newStatus === 8 ? 'Accept' : 'Decline';
  }else{
    var statusText =  newStatus === 9 ? 'Proceed to payment' : 'Cancel';
  }
  

  if (confirm("Are you sure you want to set the  " + statusText + "?")) {
  ...
  // $('#st_loader_' + id).show();
      $.ajax({
          ...
          success: function (res) {
              ...
              toastr.success( statusText + ' successfully', 'Success');
              dataTable.draw(false);
          }
      });
    } else {
  ...
  function status_add(url,newStatus, id) {
    // $('#st_loader_' + id).show();
        $.ajax({
            'headers': {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
